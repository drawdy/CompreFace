...
const reducer: ActionReducer<ModelEntityState> = createReducer(
  initialState,
  on(loadModels, createModel, uploadFace, updateModel, deleteModel, state => ({ ...state, isPending: true })),
  on(loadModelsFail, createModelFail, uploadFacesFail, uploadFacesSuccess, updateModelFail, deleteModelFail, state => ({ ...state, isPending: false })),
